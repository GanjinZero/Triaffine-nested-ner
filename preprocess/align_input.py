...
def align(input_path, output_path, version):
    ...
    for i, now_df in enumerate(df):
        ...
        if corpus_name:
            ...
            # l_idx, r_idx = get_idx(tokens, ltokens, rtokens, corpus[corpus_name])
            # new_ltokens, new_rtokens = genearte(corpus[corpus_name], l_idx, r_idx)
            # now_df['ltokens'] = ['abc']
            # now_df['rtokens'] = new_rtokens
        else:
            print(tokens)
    ...
    for corpus_name in corpus_list:
        ...
        for idx, i in enumerate(id_list):
            li = id_list[0:idx]
            ri = id_list[idx+1:]
            new_df[i]['ltokens'] = j(li, new_df)
            new_df[i]['rtokens'] = j(ri, new_df)
...
def main():
    ...
    align('../nested_v2/data/ace05/ace05_train_context.json', 'res/ace05_train_v2.json', ace05)
    align('../nested_v2/data/ace05/ace05_dev_context.json', 'res/ace05_dev_v2.json', ace05)
    align('../nested_v2/data/ace05/ace05_test_context.json', 'res/ace05_test_v2.json', ace05)
    align('../nested_v2/data/ace04/ace04_train.json', 'res/ace04_train_v2.json', ace04)
    align('../nested_v2/data/ace04/ace04_dev.json', 'res/ace04_dev_v2.json', ace04)
    align('../nested_v2/data/ace04/ace04_test.json', 'res/ace04_test_v2.json', ace04)
